...
int pause(void){
    ...
    tios.c_lflag&=~ICANON; // 关闭Canonical模式(这样无须换行即可获得输入)
    tios.c_lflag&=~ECHO;   // 关闭回显
    tios.c_cc[VMIN]=1;     // 在非Canonical模式下最少输入1字符即可读取
    tios.c_cc[VTIME]=0;    // read会一直等待输入,无超时
    if(tcsetattr(STDIN_FILENO, TCSANOW, &tios)<0) // 修改并立刻生效
        bSuccess=false;
